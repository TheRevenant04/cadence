...
- Install Python deps with `uv sync` or `uv pip install -r requirements.txt`. The API container runs via `uv run` against its project venv; `docker-entrypoint.sh` runs Alembic migrations (`uv run alembic upgrade head`) then seeds demo data (`uv run python -m app.seed`) before starting Uvicorn.
- For frontend, from `frontend/`: `npm install`, then `npm run dev` for the Vite dev server (`pnpm` is not available on the dev machine).
- Frontend scripts: `npm run dev` (Vite), `npm run build`, `npm run preview`, `npm run typecheck`, `npm test`. The compose `web` service builds `frontend/Dockerfile` (dev target), runs Vite on `0.0.0.0:5173`, and bind-mounts `./frontend/src` for hot reload.
- TanStack Router file-based routing: `frontend/src/routeTree.gen.ts` is generated from `frontend/src/routes/`. Running the generator inside the `web` container fails with `EXDEV: cross-device link not permitted` (the bind mount cannot rename across devices), so regenerate on the host instead — e.g. `node` a script that calls `Generator.run()` from `@tanstack/router-generator` with `root` = `frontend/`, or add/rename a route file and run `npm run build` on the host. After changing `frontend/src/routes/`, regenerate the tree and commit it.
- Note: the UI talks to the FastAPI backend in `backend/` (HTTP + WebSocket) through the single hub `frontend/src/api/index.ts` (HTTP client in `client.ts`). The backend base URL defaults to `http://localhost:8000` and can be overridden with `VITE_API_BASE`. A localStorage-backed mock still ships in `frontend/src/api/mock.ts` for reference/tests but is not used by the UI. PostgreSQL and the `backup` service are implemented: `db` runs `postgres:16-alpine` with named volume `db_data`; `backup` runs hourly `pg_dump` (custom-format) into volume `backup_data` via `backup/backup.sh`, pruning dumps older than 30 days.
- Run backend tests with `pytest` from the `backend/` directory; run frontend tests with `npm test` from `frontend/`.
...
- Database migrations managed via Alembic; the API container runs `alembic upgrade head` automatically at boot, so state the schema lives solely in `backend/alembic/versions/` (do not create tables via `Base.metadata.create_all` for the compose DB).
- Backups stored in a Docker volume; inspect with `docker compose exec backup ls /backups`.
- The seeded demo login is `admin@cadence.dev` / `Password123!` (see `backend/app/db.py`, `DEMO_PASSWORD`).

---

## Testing instructions

- Backend: run `pytest` from `backend/` for full test suite. Use `pytest -k <test_name>` to focus on specific tests.
...
**Current status:** the interactive frontend lives in `frontend/`. All backend calls go through the single hub `frontend/src/api/index.ts`, which talks to the Python/FastAPI backend in `backend/` over HTTP (`client.ts`) + WebSocket (`socket.ts`); session tokens are stored in localStorage. A localStorage-backed mock (`src/api/mock.ts`, seeded demo accounts) still ships for reference/tests. The backend is covered by pytest and the client by Vitest. PostgreSQL, Docker Compose, and scheduled backups are implemented and are the primary local-dev workflow (`docker compose up --build`).
